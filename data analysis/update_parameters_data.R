...
log.likelihood <- function(W.tilde.obs, Y.obs) {
  ...
  for(i in 1:N.obs){
    ...
    like.count <- like.count + dpois(Y.obs[i,1], lambda = exp(W.tilde.obs[i,1]), log = T)
...
# 
#       like.cont <-  like.cont + dnorm(Y.obs[i,1],  mean = W.tilde.obs[i,1],  1, log = T)
#       #like.count <- like.count + dnorm(Y.obs[i,2],  mean = W.tilde.obs[i,2],  1, log = T)
# 
...
update.W.tilde <- function(W.tilde.obs, beta, Sigma, K.tilde.obs,
  ...
  post.W.tilde <- log.likelihood(W.tilde.obs, Y.obs) +
                            dmatnorm(W.tilde.obs,
                                     ...
                                     V = Sigma,
                                     log = TRUE)
  W.error <- matrix(rnorm(N.obs*q, 0, sqrt(tuning.W.tilde)), nrow = N.obs, ncol = q)
...
update.nu <- function(phi, beta, Sigma, nu, r, W.tilde.obs, distmat.obs, 
  ...
  post.nu <- dmatnorm(W.tilde.obs,
                       ...
                       V = Sigma,
                       log = TRUE) + 
             dlnorm(nu, meanlog = -log(2) - 8, sdlog = 4, log = TRUE)
  ...
  if(can.nu > 0){
    ...
    post.can.nu <- dmatnorm(W.tilde.obs,
                             ...
                             V = Sigma,
                             log = TRUE) + 
                   dlnorm(can.nu,  meanlog = -log(2) - 8, sdlog = 4, log = TRUE)
