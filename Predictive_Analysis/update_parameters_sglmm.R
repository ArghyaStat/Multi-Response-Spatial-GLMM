...
log.likelihood <- function(W.tilde.obs, Y.obs) {
    ...
    lambda <- exp(W.tilde.obs[,2])
  
    
    like.count <- sum(dpois(Y.obs[,2], lambda = lambda, log = T))
    
    #like.cont <-  sum(W.tilde.obs[,1])
    ...
    # 
    # 
    # like.cont <-  like.cont + sum(dnorm(Y.obs[i,1], mean = W.tilde.obs[i,1],  1 , log = T))
    # }
...
update.W.tilde <- function(W.tilde.obs, beta, Sigma, r, phi, nu, Y.obs, X.obs, N.obs, 
                           K.tilde.obs, distmat.obs, acc.W.tilde, tuning.W.tilde){
  ...
  post.W.tilde <- log.likelihood(W.tilde.obs, Y.obs) +
                            dmatnorm(W.tilde.obs,
                                     ...
                                     V = Sigma,
                                     log = TRUE)
  W.error <- sqrt(tuning.W.tilde) * matrix(rnorm(N.obs*q, 0, 1), nrow = N.obs, ncol = q)
...
update.beta <- function(W.tilde.obs, Sigma, X.obs, K.tilde.obs.inv, M.prior, V.prior){
...
update.Sigma <- function(W.tilde.obs, beta, X.obs, K.tilde.obs.inv, 
  ...
  resid.W.tilde.obs <- W.tilde.obs - X.obs %*% beta
  rss.W.tilde.obs <- t(resid.W.tilde.obs) %*% K.tilde.obs.inv %*% resid.W.tilde.obs
  
  rss.beta <- t(beta - M.prior) %*% prec.V.prior %*% (beta - M.prior)
  
  S.tilde <- S.prior + rss.W.tilde.obs + rss.beta
  df.tilde <- df.prior + N.obs + p
