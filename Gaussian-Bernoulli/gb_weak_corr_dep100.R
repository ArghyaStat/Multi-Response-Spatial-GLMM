...
# results <- foreach(r = 1:reps, .packages = libraries, .export = functions_pred) %dopar% {
  ...
  plot.ts(phi.post, main = "Trace Plot for phi",
          xlab = "Iteration", ylab = "phi", col = "black")
  ...
  # 
  # waic.pred <- waic(W.ord.post, Y.pred.ord.true, mlpd.pred, family)
  # 
...
list.save(out, file = "gb_weak_corr_dep_est100.RData")
