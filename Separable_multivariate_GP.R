...
metropolis_hastings <- function(beta, Sigma, phi, nu, r, niters, 
  ...
  for (iter in 2:niters) {
    ...
    # Compute log posterior for the proposed value
    log.r_Sigma <- log_posterior(current_beta, 
                                current_Sigma,
                                current_phi,
                                current_nu,
                                current_r,
    ...
    # Compute log posterior for the proposed value
    log.r_phi <- log_posterior(current_beta, 
                               current_Sigma,
                               current_phi,
                               current_nu,
                               current_r,
    ...
    # Compute log posterior for the proposed value
    log.r_nu <- log_posterior(current_beta, 
                               current_Sigma,
                               current_phi,
                               current_nu,
                               current_r,
    ...
    # Compute log posterior for the proposed value
    log.r_r <- log_posterior(current_beta, 
                             current_Sigma,
                             current_phi,
                             current_nu,
                             current_r,
...
tuning_params <- c(5, 0.5, 0.05, 1e-2, 1e-2)
